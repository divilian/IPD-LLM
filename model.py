...
class IPDModel(Model):
	...
	def request_foaf_info_from(self, agent, neighbor_node):
		...
		neighbor_agent = self.node_to_agent[neighbor_node]

		#direct references to wealth to keep same order of : matches -> resolve_payoffs -> permit_rewiring
		agent.wealth -= 1 # $1 cost of request

		""" 
		for option 2 or 3: use copy.deepcopy(self.history) to create a deep copy of your history edit
		"""
		#Option 2 (Hannah variant): Did they refuse?
		if all(value is None for value in answer.values()):
			pass #receive nothing ($0)
			
		#Option 1 (Delaney variant): Did they tell the absolute truth?
		elif answer == neighbor_agent.history:
			neighbor_agent.wealth += 1
			
		#Option 3: Did they provide a history with lies/errors?
		#If it isn't completely None, and it isn't the absolute truth, it must be a lie.
		else:
			neighbor_agent.wealth -= 0.5	
